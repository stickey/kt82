...
// ───────────────────────── Tracker: grid ─────────────────────────
function TrackerGrid({ teams, onSelect, t, sinceSec }) {
  const K = window.KT82;
...
// ───────────────────────── Tracker: detail ─────────────────────────
function TrackerDetail({ team, onBack, t, tick }) {
  ...
  return (
    <div style={{ minHeight: '100%', background: t.bg, color: t.text, fontFamily: P_BODY, paddingTop: 52 }}>
      ...
      <div style={{ margin: '0 18px', background: sc, color: t.ink, borderRadius: 22, overflow: 'hidden',
        ...
        <div style={{ padding: '12px 20px 16px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontFamily: P_BODY, fontWeight: 700, fontSize: 11, opacity: 0.85, marginTop: 5, letterSpacing: '0.06em' }}>EST. ARRIVAL</div>
          </div>
      ...
      <div style={{ padding: '22px 18px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        ...
        <span style={{ fontFamily: P_BODY, fontWeight: 700, fontSize: 11, color: t.mut, letterSpacing: '0.06em' }}>18 HANDOFFS</span>
      </div>
...
function DriverTiming({ t, legIdx, legStartedAt, raceStartedAt, nowMs, onLap, onEnd }) {
  const K = window.KT82;
...
function DriverApp({ t, resetKey }) {
  ...
  useEffect(() => { setPhase('start'); setLegIdx(0); setRaceStartedAt(null); setLegStartedAt(null); setSplits([]); }, [resetKey]);
  useEffect(() => {
  ...
  return <DriverTiming t={t} legIdx={legIdx} legStartedAt={legStartedAt} raceStartedAt={raceStartedAt} nowMs={nowMs} onLap={lap} onEnd={endRace} />;
}
